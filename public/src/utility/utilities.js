...
export default function mobileAndTabletCheck() {
    ...
    console.log(`Is the user on a mobile device: ${check}`)
    return check;
  };
...
export function loadJSONFile(callback, path) {   
 ...
 // STORE DATA TO THE LOCAL BROWSER

 // fix this stuff here
 export function storeData(ref, item) {
    localStorage.setItem(ref, item);
 }

 export function getData(item) {
    let storedItem = localStorage.getItem(item);
    console.log(`the value memorised is ${storedItem}`);   
 }